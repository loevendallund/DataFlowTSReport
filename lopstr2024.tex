...
The goal of data-flow analysis is to provide flow information of
programs that can be used in compiler optimization.

The original approach is to use a iterative algorithm that represents
the control-flow of a program as a graph \cite{KildallGaryA1973Auat,
  RyderBarbara1988Idaa}.  Other graph-free approaches have also been
considered \cite{HorspoolR.Niegel2002AGAt}.

Type systems have often been used to provide a semantic analysis of
programs in order to characterize specific run-time errors. For
languages with imperative features, the treatment of aliasing becomes
important. In \cite{10.1007/3-540-46425-5_24} Smith et al. present a
notion of alias types that allows functions to specify the shape of a
store and to track the flow of pointers through a computation. The
language is a simple location-based language. Later, Ahmed et
al. \cite{DBLP:conf/tlca/MorrisettAF05} consider a language which is
...
In what follows, we will use a type system in the setting of data flow
analysis for a non-trivial fragment of the recent programming language
ReScript. This is a language based on OCaml with a JavaScript-inspired
synta.  ReScript offers a type system based on that of OCaml, which
provides an alternative to other typed languages that target
JavaScript \cite{rescript_rebrand}. Like OCaml, ReScript is imperative
and allows for mutability through reference constructs for creation,
reading, and writing,

The fragment that we study incorporates both functional and imperative
features. We show that the the type system is sound in the sense that
...
This section presents a $\lambda$-calculus fragment of ReScript, with bindings, pattern matching, and mutability, formally.
As we are interested in analysing the data-flow it is important to contain the information of where variables are used.
Therefore, the language presented is extended with the notion of program points to distinguishe parts of a program.
As such, we are calling an expression at a program point for occurrences and an expression without the notion of program points as atomic occurrence.
...
As the language we present is based on $\lambda$-calculuse, a program is given by an occurrence, wich in turns consist of sub-occurrences.

The syntactic categories for the language is given by% in our version of ReScript are
%
\begin{align*}
	e\in &\;\cat{Exp} &-\;&\mbox{Atomic Occurrence} \\
	o\in &\;\cat{Occ} &-\;&\mbox{Occurrences} \\
	c\in &\;\cat{Con} &-\;&\mbox{Constants} \\
	x,\;f\in &\;\cat{Var} &-\;&\mbox{Variables} \\
	\loc\in &\;\cat{Loc} &-\;&\mbox{Locations}
\end{align*}
%
Every occurrence consists of atomic occurrences labelled with a unique \emph{program point} taken from a countably infinte set $\cat{P}$.
We denote an occurrence as $e^p$ where $p \in \cat{P}$.
%Every subexpression is labelled with a unique \emph{program point}
%taken from a countably infinite set $\cat{P}$. We call a labelled
%expression $e^p$ where $p \in \cat{P}$ an \emph{occurrence}. An
%occurrence is \emph{atomic} if it of the form $u^p$ where
...
As occurrences are atomic occurrences labelled with a program point, we have that $\cat{Occ}=\cat{Exp}^\cat{P}$.
%As occurrences are labelled expressions, we have that $\cat{Occ}=\cat{Exp}^\cat{P}$.

The formation rules are shown below.
%
\begin{align*}
  \text{Occurrences} \; o &::= e^p \\[5mm]
\text{Atomic Occurrences} \; e &::= x \mid c \mid o_1\;o_2 \mid \lambda x.o\\
			& \mid c \; o_1 \; o_2\\
...
Here, functional constants represents functions such as $PLUS$, $DIVISION$, etc.

$o_1\;o_2$ denotes an \emph{Applications}, and $c\;o_1\;o_2$ denotes a \emph{Functional Applications}
...
An environment $env \in \cat{Env} $ is a partial function
\[ env: \cat{Var}\rightharpoonup\cat{Values} \]
%
and we let $env^{-1}(v)=\{x\in dom(env)\mid env(x)=v\}$.
%
A store $sto \in \cat{Sto}$ is a partial function
\[ \cat{Sto}=\cat{Loc}\cup\{next\}\rightharpoonup\cat{Values} \]
where $\nexte$ is a pointer to the next unused location -- this
information is needed when new locations are needed.
...
However, the type system introduced contains slack. In particular, the
...
Moroeover, if different references had different types, e.g., if it had an abstraction type at one point and a base type at another point.
