...
The original approach is to build a system of flow equations based on
a graph representation of the program and to compute a solution using
an iterative algorithm \cite{KildallGaryA1973Auat,
  RyderBarbara1988Idaa}. Other graph-free approaches have also been
considered \cite{HorspoolR.Niegel2002AGAt}. A challenge is this
setting is how to deal with aliasing.
...
In what follows, we will use a type system for data flow analysis in
the presence of aliasing for a non-trivial fragment of the recent
programming language ReScript. This is a language based on OCaml with
a JavaScript-inspired synta.  ReScript offers a type system based on
that of OCaml, which provides an alternative to other typed languages
that target JavaScript \cite{rescript_rebrand}. Like OCaml, ReScript
is imperative and allows for mutability through reference constructs
for creation, reading, and writing,
...
As we are interested in analysing the data-flow, we must record
information of where variables are used.  Therefore, the language
presented is extended with a notion of \emph{program points} taken
from a countably infinite set $\cat{P}$. 
%
\begin{align*}
...
\end{align*}
%
Every subexpression is labelled with a unique \emph{program point}. We call a labelled
