...
\begin{itemize}
	...
	\item We also needs to ensure that $\Pi$ is a good
	...
	\eqref{prop:5} ensures that if an order is defined in
        $\sqsubseteq_w$, then $\Pi$ also agrees on this order. 

	For \eqref{prop:6}, we need to ensure that for any location
        currently known the exists a corresponding internal variable
