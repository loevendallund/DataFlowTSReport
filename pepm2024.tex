...
The implementation follows the type rules found in \cref{fig:ColSem}, and only diverge in how abstractions and applications are handled.
...
\bigskip
To verify the implementation, we ran it against the examples presented earlier in this paper.
All of the examples parsed and got typed correctly.
...
In the following listing, we show how we approximate applications.
Here we expect that the first occurrence, $o_1$, returns an occurrence that can be approximated, as the program points from the body of the abstraction, that $o_1$ returned, are after both $o_1$ and $o_2$.
