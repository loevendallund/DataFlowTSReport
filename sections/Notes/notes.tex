	...
	\begin{description}
		...
		\begin{figure}[H]
			...
			\begin{tabular}{l}
					...
					env[x\mapsto v'] \vdash \left\langle e_3^{p_1},sto'',w'',p'' \right\rangle \rightarrow \left\langle v'',sto_3,w_3,(L'',V''),p_1 \right\rangle}
			...
			\end{tabular}
		\end{figure}
		Where $v=\left\langle y,e_3^{p_1},env\right\rangle$.
...
\begin{proof}
	...
	\begin{description}
		...
		This case follows immediately as $x\notin fv(c^p)$

		\item[\runa{Var}] Here $e^p=y^p$, where
		$$\Gamma,\Pi\vdash y^p : T$$
		Where $\Lambda{y,p}=p'$ and $\Gamma(y^{p'}=T)$.
		This case follows immediately as $x\notin fv(c^p)$
	\end{description}
	Next, follows the induction step:
\end{proof}
