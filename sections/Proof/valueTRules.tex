...
For the sake of proving the type system, we present type rules for values, where the type rules is given in \cref{fig:ValTypeRules}.

As the values for closures and recursive closures contains the environment from where they where declared, as such, before introducing the type rules for values we must define when the environment is well-typed.
...
In \cref{def:TEnv} we show when environments $env$ are well-typed.
This definition states that given the type of all values that a variable is bound to in the $env$, then for a type environment, the $env$ is well typed if when we look up the value of all variables $x$ in $env$, 
then there also exists an occurrence for $x$ in the type environment that agrees with the given type.
...
\begin{description}
	...
	\item[\runa{Closure}] type rule represents abstraction, and as such we know that it should have the abstraction type, $T_1\rightarrow T_2$, where we need to assume that the argument of the type $T_1$.
	...
	\item[\runa{Unit}] type rule simply have the base type, as it is not an abstraction and it also cannot have alias information.
		As the unit value is introduced from writing to references, $o=[o_1\;:=\;o_2]^p$, we know that from the type rule \runa{Ref-write} that the dependencies from the occurrence $o$ is the dependencies from $o_1$.
		As such \runa{Unit} should also reflect this, where the set of occurrences $\delta$ can be non-empty.
\end{description}
