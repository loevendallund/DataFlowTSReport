...
\end{figure}
Where $\kappa=\{\nu x\}$.
We need to show that \cat{1)} $\Gamma,\Pi\vdash [\lambda\;x.e^{p''}]:T$, \cat{2)} $(env,sto',(w',\sqsubseteq_w'))\models(\Gamma,\Pi)$, and \cat{3)} $(env,(w',\sqsubseteq_w'),v,(L,V))\models(\Gamma,T)$.
...
\begin{description}
	...
	\item[2)] Since we know that $(env,sto'',(w'',\sqsubseteq_w'))\models(\Gamma,\Pi)$, we then need to show for the extension to $sto''$, $w''$, and $\Gamma'$.
		Due to \cat{1)}, $(env,(w'',\sqsubseteq_w'),(L,V))\models(\Gamma,(\delta',\kappa'))$, and since we bind $\loc^{p'}$ in $sto''$ and $\nu x^{p'}$ in $\Gamma$, we can then conclude that $(env,sto',(w',\sqsubseteq_w'))\models(\Gamma',\Pi)$
	\item[3)] Due to \cat{1)} and \cat{2)} we can conclude that $(env,(w',\sqsubseteq_w'),v,(L,V))\models(\Gamma,T)$.
\end{description}
