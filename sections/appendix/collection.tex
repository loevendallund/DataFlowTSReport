...
\begin{figure}[H]
	...
	\begin{tabular}{l}
		...
		\runa{match 1}\\[0.2cm]
			...
			Where $match(v,s)=\sigma$, $\phi=match_w(s,(L,V))$ and $w'=w\phi$\\[1cm]

		\runa{match 2}\\[0.2cm]
			\inference[]
				{env\sigma \vdash \left\langle e^{p'},sto,w',p \right\rangle \rightarrow \left\langle v,sto',w'',(L,V),p' \right\rangle}
				{env\vdash \left\langle [(v,(L,V),(s\;e^{p'}))]^{p_3},sto,w',p \right\rangle \rightarrow \left\langle v,sto',w'',(L,V),p_3 \right\rangle}\\
			Where $match(v,s)=\sigma$, $\phi=match_w(s,(L,V))$ and $w'=w\phi$\\[1cm]
